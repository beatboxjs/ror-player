...
<script lang="ts" setup>
	...
	const loading = ref<number | undefined>();
	...
	const downloadMP3 = async () => {
		showAlert({ title: "Error exporting MP3", message: 'Test', variant: "danger" });
		return;
		...
		try {
			...
			if (blob)
				FileSaver.saveAs(blob, props.tuneName + ".mp3");
		} catch(err: any) {
			loading.value = undefined;
			// eslint-disable-next-line no-console
			console.error("Error exporting MP3", err.stack || err);
			showAlert({ title: "Error exporting MP3", message: err.message, variant: "danger" });
		}
	};
