<b-modal :title="title" @ok="submit()" :ok-disabled="exists || !changed" :id="id">
	<b-form class="form-horizontal" @submit.prevent="submit(); $bvModal.hide(id)">
		<b-form-group label-cols-sm="4" label="New name" :state="!changed || exists ? 'invalid' : 'valid'" :invalid-feedback="!changed ? 'Please enter a new name for the break.' : exists ? 'This name is already taken. Please type in a different name.' : ''">
			<b-form-input v-model="newPatternName" :state="!changed || exists ? 'invalid' : 'valid'" autofocus />
		</b-form-group>
