			...
			<b-collapse :visible="isOpened[tune.tuneName]" :id="tune.collapseId">
				<b-card-body>
					<PatternPlaceholder v-for="pattern in tune.patterns" :key="pattern.patternName" :tune-name="tune.tuneName" :pattern-name="pattern.patternName" :draggable="true">
						<PatternPlaceholderItem><a href="javascript:" :title="`Copy${pattern.isCustom ? '/Move/Rename' : ''} break`" v-b-tooltip @click="copyPattern(tune.tuneName, pattern.patternName)"><i class="fas fa-copy"></i></a></PatternPlaceholderItem>
						<PatternPlaceholderItem v-if="pattern.isCustom"><a href="javascript:" title="Remove" v-b-tooltip @click="removePatternFromTune(tune.tuneName, pattern.patternName)"><i class="fas fa-trash"></i></a></PatternPlaceholderItem>
						<slot :tune-name="tune.tuneName" :pattern-name="pattern.patternName"/>
					</PatternPlaceholder>
					<div class="tune-actions">
						<a href="javascript:" @click="createPatternInTune(tune.tuneName)" title="New break" v-b-tooltip><i class="fas fa-plus"></i></a>
						<a v-if="tune.isCustom" href="javascript:" @click="renameTune(tune.tuneName)" title="Rename tune" v-b-tooltip><i class="fas fa-pen"></i></a>
						<a href="javascript:" @click="copyTune(tune.tuneName)" title="Copy tune" v-b-tooltip><i class="fas fa-copy"></i></a>
						<a v-if="tune.isCustom" href="javascript:" @click="removeTune(tune.tuneName)" title="Remove tune" v-b-tooltip><i class="fas fa-trash"></i></a>
					</div>
				</b-card-body>
			</b-collapse>
		</b-card>
