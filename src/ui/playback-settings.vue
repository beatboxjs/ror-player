<script lang="ts" setup>
	...
	const emit = defineEmits<{
		(e: 'update:playbackSettings', playbackSettings: PlaybackSettings): void;
	}>();
	...
	const playbackSettings = computed({
		...
		set: (playbackSettings) => {
			emit('update:playbackSettings', playbackSettings);
		}
	});
	...
	const allMuted = computed({
		get: () => config.instrumentKeys.every((instr) => props.playbackSettings.mute[instr]),
		set: (mute) => {
...
<template>
	<Popover custom-class="bb-playback-settings" variant="light" title="Playback settings">
		<template #button>
